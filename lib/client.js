...
function handle(msg) {
  ...
  if (to && intent) {
    ...
    try {
      ...
      // If the reply is a promise, wait for it to resolve
      reply_promise = new Promise(function(resolve, reject) {
        if (reply instanceof Promise) {
          reply.then((resolved_reply) => {
            resolve(resolved_reply)
      ...
      // Once we have the resolved reply, format and emit it
      reply_promise.then((resolved_reply) => {
        // try JSON or made-JSON output
        reply = correctReply(resolved_reply, msg)
